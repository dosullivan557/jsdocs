...
function buildNav(members) {
    let globalNav;
    const {name:repoName, version:appVersion}= JSON.parse(
        fs.readFileSync(path.join(process.cwd(), 'package.json'), 'utf8')
      )
    let nav = `<h2><a href="index.html">${repoName}@${appVersion} Documentation</a></h2>`;
    const seen = {};
    const seenTutorials = {};
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    generate(`${repoName}@${appVersion} Documentation`,
        packages.concat(
            [{
                kind: 'mainpage',
                readme: opts.readme,
